# Design Document: Where Linear Features Break in Small Transformers

## 1. Purpose

This document is the method contract for **Where Linear Features Break in Small Transformers**. It specifies what is
...
One-liner: Measure when residual features stay linear under intervention and when they don't.

Hypothesis: Measure when residual features stay linear under intervention and when they don't.
...
|---|---|---|
| Config schema | `linrep.configs.schema` | Typed dataclasses + role-keyed models |
| Loader | `linrep.configs.loader` | Hydra compose API |
| Device | `linrep.models.device` | MPS/CUDA/CPU resolution + fallback flag |
| Registry | `linrep.models.registry` | Pinned revisions + hardware fit |
| Hooks | `linrep.models.hooks` | Capture, steer, ablate, patch, attn intervene |
| Generation | `linrep.models.generation` | Chat template path recording |
| Data | `linrep.data` | Synthetic items, splits, manifests |
| Evaluation | `linrep.evaluation` | Metrics, failure harness, protocols |
| Ablation | `linrep.ablation` | Control / dropout / seed-sweep arms |
| Reporting | `linrep.reporting` | Aggregate → MD/TeX/figures |
| Cache | `linrep.cache` | Versioned artifact store |
...
`src/linrep/linrep/` are preserved across regeneration.
